...
export default function NewStudentModal({ isOpen, onClose }: Props) {
    ...
    const handleSaveStudent = () => {
        ...
        const splitC = courses.split(",")
        let crns: number[] = [];
        for (const crn of splitC) {
            const parsed = parseInt(crn.trim());
            if (isNaN(parsed)) {
                alert(`Invalid CRN input: "${crn}" is not a number.`)
                return;
            }
            crns.push(parsed);
        }
    ...
    const canSave = studentData.fullname.length > 1 && studentData.email.length > 1 && studentData.studentid.length > 1 && studentData.timeextension >= 1;
    ...
    return (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
            <div className="bg-gradient-to-b from-gray-700 text-center to-gray-800 rounded-lg shadow-lg p-6 min-w-3/8 min-h-1/2 max-w-md">
                <h2 className="text-3xl text-white font-semibold mb-4">New Student</h2>
                {/* <select
                    value={editing.crn}
