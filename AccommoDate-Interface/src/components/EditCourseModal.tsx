...
export default function EditCourseModal({ isOpen, editing, onClose }: Props) {
    const [courseData, setCourseData] = useState<Course>(editing);
    ...
    const handleDeleteCourse = () => {
        const message = "Are you sure you want delete this course?";

        const confirmed = window.confirm(message);

        if (!confirmed) return;

        fetch(`http://localhost:8080/api/course/delete/${editing.crn}`, {
            method: "DELETE",
            headers: {
                "Authorization": `Bearer ${getToken()}`,
            },
    ...
    return (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
            <div className="bg-gradient-to-b from-gray-700 text-center to-gray-800 rounded-lg shadow-lg p-6 min-w-3/8 min-h-1/2 max-w-md">
                <h2 className="text-3xl text-white font-semibold mb-4">Edit Course</h2>
                {/* <select
                    value={editing.crn}
